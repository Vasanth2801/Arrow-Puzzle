...
public static class LevelGenerator
{
    ...
    private static List<Vector2Int> BuildPathBackwards(Vector2Int head,Vector2Int headDirection,bool[,] occupied,int width,int height,int maxPathLength)
    {
        ...
        while (path.Count < maxPathLength)
        {
            ...
            Vector2Int previousDirection = current - path[path.Count - 2];

            Vector2Int chosen;

            if (candidates.Contains(previousDirection) && Random.value < 0.65f)
            {
    ...
    private static bool HasDirectionVariety(List<PathData> paths)
    {
        bool up = false;
        bool down = false;
        bool left = false;
        ...
        for (int i = 0; i < paths.Count; i++)
        {
            ...
            Vector2Int direction = path.Head - path.cells[path.cells.Count - 2];
